...
public class KeycloakUsuarioRegistrationService {
    ...
    private String extractAccessToken(String responseBody) {
        try {
            return objectMapper.readTree(responseBody).path("access_token").asText(null);
        } catch (Exception e) {
            log.error("Erro ao extrair access token", e);
            return null;
        }
    }
    ...
    private String extractFirstField(String json, String fieldName) {
        try {
            ...
            if (node.isArray()) {
                ...
                return node.get(0).path(fieldName).asText(null);
            }
            return node.path(fieldName).asText(null);
        } catch (Exception e) {
            log.error("Erro ao extrair campo do JSON: {}", fieldName, e);
            return null;
        }
    }
    ...
    private String rolesToJson(List<KeycloakRole> roles) {
        try {
            ...
            log.error("Erro ao converter roles para JSON", e);
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "Erro ao serializar perfis");
        }
    }
